...
object WmsSparkPull {
  def main(args: Array[String]): Unit = {
    ...
    for (table_name <- tablelist) {
      ...
      val finish_table_name = s"bdp_tmp.complite_pull_table_${appname}"
      try {
        spark.table(finish_table_name)
      } catch {
        case e: AnalysisException => {
          //先插入一条测试数据
          Seq(("test", new Date().getTime.toString()))
            .toDF("tableName", "currenttime")
            .write.mode("append").saveAsTable(finish_table_name)
        }
      }
      val finish_table: DataFrame = spark.sql(s"select tableName from ${finish_table_name}")
      val tables: Array[String] = finish_table.rdd.collect().map(tables => tables.toString().replace("[", "").replace("]", ""))
      val table_names = table_name.split(",")
      val name = table_names(0).replaceAll("gyl_wms_city_", "").replaceAll("_tmp", "")
      println(s"${new Date().toString} ${preff} 正在处理表：${name}")
      if (!tables.contains(name)) {
        connectOracle(name, spark, table_names(3), table_names(4), table_names(5), table_names(6))
        Seq((name, new Date().getTime.toString())).toDF("tableName", "currenttime").write.mode("append").saveAsTable(finish_table_name)
      }
      println(s"${new Date().toString} ${preff} 处理完成表：${name}")
      count = count + 1
    }
  }
  ...
  def connectOracle(table_name: String, spark: SparkSession, onlineOrHistory: String, partitionName: String, lowerBound: String, upperBound: String): Unit = {
    ...
    val dbtable = "usr_wms_city." + table_name.replace("_old", "")
    import org.apache.spark.sql.functions._
    val online_talbe_202 = getDataFrame(oracleDriverUrl_202, user_online, password_online, dbtable, driver, spark, partitionName, lowerBound, upperBound).withColumn("spark_isonline", lit("1"))
    val online_talbe_204 = getDataFrame(oracleDriverUrl_204, user_online, password_online, dbtable, driver, spark, partitionName, lowerBound, upperBound).withColumn("spark_isonline", lit("1"))
    val online_talbe_206 = getDataFrame(oracleDriverUrl_206, user_online, password_online, dbtable, driver, spark, partitionName, lowerBound, upperBound).withColumn("spark_isonline", lit("1"))
    val online_talbe_215 = getDataFrame(oracleDriverUrl_215, user_online, password_online, dbtable, driver, spark, partitionName, lowerBound, upperBound).withColumn("spark_isonline", lit("1"))
    ...
    def historyDataFrame = {
      val file = Source.fromFile("table_databases.txt")
      ...
      for (database <- databases) {
        ...
        val oracle_df = getDataFrame(oracleDriverUrl, user, password, database_table, driver, spark, partitionName, lowerBound, upperBound).withColumn("spark_isonline", lit("0"))
    ...
    spark.sql("drop table if EXISTS " + "bdp_tmp.gyl_wms_city_" + table_name + "_tmp")
